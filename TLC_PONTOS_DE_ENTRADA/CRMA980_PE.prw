...
Local oAPICliente   as object
Local lOk           as logical

//Local lXIDTLC		:= (SA1->(FieldPos('A1_XIDTLC')) > 0)
...
/*
		ApMsgInfo("Adicionando um botão na barra de botões da rotina (PE '" + cIDPonto + "')." + CRLF +;
				  "ID '" + cIDModel + "'")
*/
//		xRet	:= {{'Consulta Telecontrol',;								// Titulo para o botão
...
/*
		...
		__LogTela   := NIL
*/
		oAPICliente := Telecontrol.Integracao.Cliente.TTLCCliente():New()
		if nOpc == MODEL_OPERATION_INSERT // inclusão
			if isBlind()
				oAPICliente:Cadastra()
			else
				FwMsgRun(NIL, {|| lOk := oAPICliente:Cadastra()}, "Telecontrol", "Gravando informações...")
				if !lOk
		...
		elseif nOpc == MODEL_OPERATION_UPDATE // alteração	
			...
			cCodRet := '500'
			if !oAPICliente:Consulta(,@cCodRet,,.F.,.F.) .OR. (cCodRet <> '200') // se não existia, então inclui
				if isBlind()
					oAPICliente:Cadastra()
				else
					FwMsgRun(NIL, {|| lOk := oAPICliente:Cadastra()}, "Telecontrol", "Gravando informações...")
			...
			elseif (cCodRet == '200') // se já existia, então altera
				if isBlind()
					...
					FwMsgRun(NIL, {|| lOk := oAPICliente:Altera()}, "Telecontrol", "Atualizando informações...")
					if !lOk
